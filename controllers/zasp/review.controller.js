const ZaspReview = require('../../models/zasp/review.article');
...
exports.getSingleReview = async (req, res) => {
  try {
    ...
    const review = await ZaspReview.findOne({ slug });
...
exports.getAllReviews = async (極端, res) => {
  try {
    ...
    const [reviews, total] = await Promise.all([
      ZaspReview.find().sort({ published_at: -1 }).skip(skip).limit(limit),
      ZaspReview.countDocuments(),
    ]);
...
exports.getReviewsByTag = async (req, res) => {
  try {
    ...
    const [reviews, total] = await Promise.all([
      ZaspReview.find({ tags: tag })
        .sort({ published_at: -1 })
        .skip(skip)
        .limit(limit),
      ZaspReview.countDocuments({ tags: tag }),
    ]);
...
exports.getReviewsByVenue = async (req, res) => {
  try {
    ...
    const [reviews, total] = await Promise.all([
      ZaspReview.find({ venue })
        .sort({ published_at: -1 })
        .skip(skip)
        .limit(limit),
      ZaspReview.countDocuments({ venue }),
    ]);
...
exports.deleteReview = async (req, res) => {
  try {
    const { id } = req.params;
    const review = await ZaspReview.findByIdAndDelete(id);
