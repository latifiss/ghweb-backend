const { NigArticle } = require('../../models/nigpost/article.model');
...
exports.createArticle = async (req, res) => {
  try {
    ...
    const article = new NigArticle({
      ...
      image_url: imageUrl,
      meta_title: NigArticle.prototype.generateMetaTitle(title),
      meta_description: NigArticle.prototype.generateMetaDescription({
        title,
        description,
      }),
    });
...
exports.updateArticle = async (req, res) => {
  try {
    ...
    const existingArticle = await NigArticle.findById(id);
    if (!existingArticle) {
    ...
    const article = await NigArticle.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true,
    });
...
exports.getHeadline = async (req, res) => {
  try {
    ...
    const headlineArticle = await NigArticle.findOne({ isHeadline: true }).sort(
      {
        published_at: -1,
      }
    );
    ...
    const similarArticles = await NigArticle.find({
...
exports.getCategoryHeadline = async (req, res) => {
  try {
    ...
    const categoryHeadline = await NigArticle.findOne({
...
exports.addLiveUpdate = async (req, res) => {
  try {
    ...
    const article = await NigArticle.findById(id);
    if (!article) {
      return res.status(404).json({
        status: 'fail',
        message: 'Article not found',
...
exports.endLiveArticle = async (req, res) => {
  try {
    ...
    const article = await NigArticle.findByIdAndUpdate(
      id,
...
exports.getArticles = async (req, res) => {
  try {
    ...
    const [articles, total] = await Promise.all([
      NigArticle.find().sort({ published_at: -1 }).skip(skip).limit(limit),
      NigArticle.countDocuments(),
    ]);
...
exports.getArticleById = async (req, res) => {
  try {
    ...
    const article = await NigArticle.findOne({ slug });
...
exports.getArticlesByCategory = async (req, res) => {
  try {
    ...
    const [articles, total] = await Promise.all([
      NigArticle.find({ category })
        .sort({ published_at: -1 })
        .skip(skip)
        .limit(limit),
      NigArticle.countDocuments({ category }),
    ]);
