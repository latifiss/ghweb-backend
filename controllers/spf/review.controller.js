const SpfReview = require('../../models/spf/review.model');
...
exports.getSingleReview = async (req, res) => {
  try {
    ...
    const review = await SpfReview.findOne({ slug });
...
exports.getAllReviews = async (極端, res) => {
  try {
    ...
    const [reviews, total] = await Promise.all([
      SpfReview.find().sort({ published_at: -1 }).skip(skip).limit(limit),
      SpfReview.countDocuments(),
    ]);
...
exports.getReviewsByTag = async (req, res) => {
  try {
    ...
    const [reviews, total] = await Promise.all([
      SpfReview.find({ tags: tag })
        .sort({ published_at: -1 })
        .skip(skip)
        .limit(limit),
      SpfReview.countDocuments({ tags: tag }),
    ]);
...
exports.getReviewsByVenue = async (req, res) => {
  try {
    ...
    const [reviews, total] = await Promise.all([
      SpfReview.find({ venue })
        .sort({ published_at: -1 })
        .skip(skip)
        .limit(limit),
      SpfReview.countDocuments({ venue }),
    ]);
...
exports.deleteReview = async (req, res) => {
  try {
    const { id } = req.params;
    const review = await SpfReview.findByIdAndDelete(id);
