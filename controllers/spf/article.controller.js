...
exports.createArticle = async (req, res) => {
  try {
    ...
    const article = new SpfArticle({
      ...
      image_url: imageUrl,
      meta_title: SpfArticle.prototype.generateMetaTitle(title),
      meta_description: SpfArticle.prototype.generateMetaDescription({
        title,
        description,
      }),
    });
...
exports.addLiveUpdate = async (req, res) => {
  try {
    ...
    const article = await SpfArticle.findById(id);
    if (!article) {
      return res.status(404).json({
        status: 'fail',
        message: 'Article not found',
...
exports.endLiveArticle = async (req, res) => {
  try {
    ...
    const article = await SpfArticle.findByIdAndUpdate(
      id,
...
exports.getArticles = async (req, res) => {
  try {
    ...
    const [articles, total] = await Promise.all([
      SpfArticle.find().sort({ published_at: -1 }).skip(skip).limit(limit),
      SpfArticle.countDocuments(),
    ]);
...
exports.getArticleById = async (req, res) => {
  try {
    ...
    const article = await SpfArticle.findOne({ slug });
...
exports.getArticlesByCategory = async (req, res) => {
  try {
    ...
    const [articles, total] = await Promise.all([
      SpfArticle.find({ category })
        .sort({ published_at: -1 })
        .skip(skip)
        .limit(limit),
      SpfArticle.countDocuments({ category }),
    ]);
